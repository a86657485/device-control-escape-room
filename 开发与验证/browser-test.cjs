...
(async()=>{
 ...
 await p.locator('[data-action="hint"]').click();assert.match(await p.locator('.hint-box').innerText(),/阻碍阅读/);
 ...
 for(let r=0;r<6;r++){
  for(let s=0;s<3;s++){
   ...
   if(r===1){if(s===0){assert.doesNotMatch(await p.locator('.stage').innerText(),/需要强风/);await p.locator('[data-action="hint"]').click();await p.locator('[data-action="hint"]').click();await p.locator('[data-action="hint"]').click();assert.match(await p.locator('.hint-btn').innerText(),/2\/3/);assert.doesNotMatch(await p.locator('.hint-box').innerText(),/3挡/);await toggle('fan',true);await set('speed',1);await p.locator('#prediction').selectOption({index:2});await p.locator('[data-action="run"]').click();await p.locator('.feedback').filter({hasText:'叶轮转动缓慢'}).waitFor();assert.doesNotMatch(await p.locator('.feedback').innerText(),/3挡|本次需要/);await p.locator('[data-action="hint"]').click();assert.match(await p.locator('.hint-box').innerText(),/3挡/);}await toggle('fan',true);await set('speed',s===0?3:1);if(s===2)await set('timer',30);}
   ...
   await p.locator('[data-action="next-step"]').click();if(r===0&&s===0){assert.equal(await p.locator('.hint-box').count(),0);assert.match(await p.locator('[data-action="hint"]').innerText(),/0\/3/);}
  }
 ...
 for(let group=0;group<4;group++){
  await p.locator('[data-action="assessment-page"][data-page="'+group+'"]').first().click();assert.equal(await p.locator('.assessment-item').count(),4);
  if(group>=2){await p.locator('.extension-reading summary').click();assert.match(await p.locator('.extension-reading').innerText(),/辘轳/);}
  for(const item of finalAssessment.filter(q=>q.group===group)){
   const choice=item.id==='deviceFunction'?(item.correct+1)%3:item.correct;
   const target=p.locator('[data-action="assessment-answer"][data-id="'+item.id+'"][data-value="'+choice+'"]');
   await target.click();assert.equal(await p.evaluate(()=>document.activeElement.dataset.id),item.id);
  }
 }
 for(const [key,value] of [['transferDevice','电风扇'],['transferFunction','煮粥'],['transferMethod','打开电源并选择风速'],['transferEvidence','观察气流是否让机关移动']])await p.locator('[data-action="transfer-answer"][data-id="'+key+'"][data-value="'+value+'"]').click();
 await p.locator('.topbar [data-action="notebook"]').click();assert.match(await p.locator('.assessment-report').innerText(),/作答草稿未评分/);assert.doesNotMatch(await p.locator('.assessment-report').innerText(),/（合理）|（待调整）|搭配合理/);await p.getByRole('button',{name:'关闭对话框',exact:true}).click();
 assert.equal(await p.locator('[data-action="assessment-submit"]').isDisabled(),false);await p.locator('[data-action="assessment-submit"]').click();assert.doesNotMatch(await p.locator('.assessment-result').innerText(),/考核通过/);assert.match(await p.locator('.assessment-result').innerText(),/首答 15\/16/);
 await p.reload();assert.match(await p.locator('.assessment-result').innerText(),/首答 15\/16/);
 await p.locator('[data-action="assessment-page"][data-page="0"]').first().click();
 const first=finalAssessment[0];await p.locator('[data-action="assessment-answer"][data-id="'+first.id+'"][data-value="'+first.correct+'"]').click();
 await p.locator('[data-action="assessment-page"][data-page="3"]').first().click();await p.locator('[data-action="transfer-answer"][data-id="transferFunction"][data-value="促进空气流动"]').click();
 await p.locator('[data-action="assessment-submit"]').click();assert.match(await p.locator('.assessment-result').innerText(),/任务单考核通过/);assert.match(await p.locator('.assessment-result').innerText(),/首答 15\/16；当前 16\/16/);
 await p.screenshot({path:path.join(out,'08-通关成果.png'),fullPage:true});
 ...
 await p.getByRole('button',{name:'关闭对话框',exact:true}).click();await p.locator('.topbar [data-action="notebook"]').click();await p.locator('#team-name').fill('星光测试组');assert.equal(await p.locator('#reflection').count(),0);
 const downloadPromise=p.waitForEvent('download');await p.locator('#modal [data-action="export"]').click();const dl=await downloadPromise;await dl.saveAs(path.join(out,'学习证据报告-测试.html'));const report=fs.readFileSync(path.join(out,'学习证据报告-测试.html'),'utf8');assert.match(report,/同伴试验1次；通过是/);assert.match(report,/星光测试组/);assert.match(report,/首答：15\/16/);assert.match(report,/最近提交：16\/16/);assert.match(report,/辘轳/);
 ...
 assert.deepEqual(errors,[]);assert.deepEqual(requests,[]);fs.writeFileSync(path.join(out,'测试结果.json'),JSON.stringify({result:'PASS',mainTasks:18,explanationChecks:6,taskSheetAssessment:true,assessmentItems:16,firstAndRevisedScoresPreserved:true,transferPairingChecked:true,scenarioHints:true,draftAnswersNotGraded:true,thirdHintRequiresTrial:true,noLongTextRequired:true,teacherGuideHidden:true,wrongOperationBlocked:true,wrongExplanationBlocked:true,timerStops:true,refreshResumesPending:true,keyboardCodeFocus:true,trafficAnimationObeysSignal:true,peerEvidenceSurvivesNoteEdit:true,exportVerified:true,viewports:[1440,768,390],storageFailureHandled:true,externalRequests:requests,pageErrors:errors},null,2));console.log('PASS: 18操作/6解释/16项终局任务单判断，选项拼句、错误分支、定时停止、刷新恢复、键盘、同伴验证、导出、窄屏、禁用存储、零外部请求。');
